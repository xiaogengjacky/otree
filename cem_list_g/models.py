...
class Group(BaseGroup):
    ...
    def set_group_payoffs(self):
        ...
        # ------------------------------------------------------------------------------------------------------------
        for p in self.get_players():
            p.participant.vars['cem_choices_group'] = [1 if j == 'A' else 0 for j in
                                                       p.participant.vars['cem_choices_made'][self.random_list - 1]]

        # implement voting outcome for the group
        # ------------------------------------------------------------------------------------------------------------

        group_voting = [p.participant.vars['cem_choices_group'] for p in self.get_players()]
        group_voting = [sum(x) for x in zip(*group_voting)]
        ...
        for j in range(0, n):
            if group_voting[j] <= (group_size - 1)/2:
                p.participant.vars['cem_choices_group'][j] = ['B' for p in self.get_players()]
            else:
                p.participant.vars['cem_choices_group'][j] = ['A' for p in self.get_players()]

        if group_voting[self.choice_to_pay - 1] <= (group_size - 1)/2:
            self.option_to_pay = 'B'
        else:
            self.option_to_pay = 'A'
